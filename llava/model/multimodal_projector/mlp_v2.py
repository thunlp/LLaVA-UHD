...
class MLP_v2(nn.Module):
    ...
    def __init__(
        ...
        self.conv_pool = nn.Conv2d(kv_dim, kv_dim, kernel_size=2, stride=2)

        self.proj = nn.Sequential(
            nn.GELU(),
            nn.Linear(kv_dim * 2 , embed_dim),
            nn.GELU(),
    ...
    def prepare_resized_feat(self, feature_1x, feature_2x, feature_4x, feature_8x, tgt_size=(24, 24), attn_mask=None, dtype=torch.bfloat16):
        ...
        for idx in range(len(feature_list)):
            ...
            feature = F.interpolate(feature, size=(12, 12), mode='bilinear', align_corners=False)
        ...
        resized_feat = resized_feat.reshape(resized_feat.shape[0], resized_feat.shape[1], -1)
        resized_feat = resized_feat.permute(0,2,1)

        resized_feat = self.proj(resized_feat)
        
        resized_feat = resized_feat[0]
        #torch.Size([1, 2048, 24, 24])
        return resized_feat
