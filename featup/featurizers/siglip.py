...
class SigLipVisionEmbeddings(nn.Module):
    ...
    def resize_pos_embedding(self, position_embedding, dst_size=(27, 27), square_size=27):
        _dtype = position_embedding.dtype
        patch_height, patch_width = dst_size

        position_embedding = position_embedding.permute(0, 2, 1).unflatten(-1, [square_size, square_size])
        ...
        position_embedding = position_embedding.flatten(-2).permute(0, 2, 1) # 1, n, c
        return position_embedding
    
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        patch_embeds = self.patch_embedding(pixel_values)  # shape = [*, width, grid, grid]
        embeddings = patch_embeds.flatten(2).transpose(1, 2) #torch.Size([1, 1152, 27, 27])
...
class SigLipVisionTower(nn.Module):
    def __init__(self):
        ...
        self.vision_tower_name = '/mnt/data/user/tc_agi/zyp/llavafeatup/model_checkpoints/google-siglip-so400m-patch14-384'
    ...
    def forward(self, images):
        ...
        #torch.Size([7, 729, 1152])
        #torch.Size([1, 576, 1024])
        features = image_features.permute(0,2,1) #torch.Size([50, 1152, 729])
        #[1, 1024, 24, 24]
        features = features.reshape(len(features), features.shape[1], 27, 27)
        return features
    
    def forward_uhd_v2(self, images):
        if type(images) is list:
            image_features = []
            for image in images:
                image_forward_out = self.vision_tower(image.to(device=self.device, dtype=self.dtype).unsqueeze(0), output_hidden_states=True)
